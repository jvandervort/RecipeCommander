...
public class BookUtils {
  ...
  /** 
   * Loads the jtree on the main screen.  Includes support for filtering and sorting.
    ...
    if (filter != null) {
      ...
      if (viewBy.equals("chapter")) {
        ...
        while (it.hasNext()) {
          ...
          // if group by chapter, but also should support group by region, and maybe others
          ...
          if (rec.getCat().compareTo("") != 0 && chapterNode != null && lastrec.getCat().compareTo(rec.getCat()) != 0) {
            catNode = new DefaultMutableTreeNode(rec.getCat());
            chapterNode.add(catNode);
            subCatNode = null;
          }

          if (rec.getSubCat().compareTo("") != 0 && catNode != null && lastrec.getSubCat().compareTo(rec.getSubCat()) != 0) {
        ...
        while (it.hasNext()) {
          ...
          if (rec.getChapter().compareTo("") != 0 && originNode != null && lastrec.getChapter().compareTo(rec.getChapter()) != 0) {
        ...
        while (it.hasNext()) {
          ...
          if (rec.getChapter().compareTo("") != 0 && sourceNode != null && lastrec.getChapter().compareTo(rec.getChapter()) != 0) {
            // they are different, add chapter node
            chapterNode = new DefaultMutableTreeNode(rec.getChapter());
...
class TreeComparator implements Comparator<Recipe> {
  ...
  public int compare(Recipe o1, Recipe o2) {
    ...
    if (by.equals("chapter")) {
      ...
      compareVal = r1.getTreeAncestors(TreeViewBy.recipeName).compareTo(r2.getTreeAncestors(TreeViewBy.recipeName));
    }
