...
public class FileHistory {
	...
	/**
	 * Interface that must be implemented by a GUI applcation frame that wants to
	 * use the FileHistory class.
	 */
	...
	public FileHistory(IFileHistory caller) {
		this.caller = caller;
		historySerFile = System.getProperty("user.home") + FILE_SEPARATOR_STR + caller.getApplicationName()
				+ "_FILE_HISTORY.cfg";
	...
	/*******************************************************************
	 * Initialize itemname and pathname arraylists from historySerFile. * build up
			...
			try {
				...
				for (int i = 0; i < itemnameCount; i++) {
					...
					MenuItemWithFixedTooltip item = new MenuItemWithFixedTooltip(
							(i + 1) + ":" + (String) itemnameHistory.get(i));
	...
	public final void insertPathname(String pathname) {
		...
		// loop through and see if the pathname exists, if so, delete it and call
		// insertPathname again
	...
	/**********************************************************
	 * Return the itemname (abbreviated itemname if necessary) * to be shown in the
	 * file menu open item list. * A maximum of MAX_ITEM_LEN characters is used for
	 * the * itemname because we do not want to make the JMenuItem * entry too wide.
	 * *
	 **********************************************************/
	protected String getItemname(String pathname) {
		...
		if (pathname.indexOf(FILE_SEPARATOR_STR) == pathname.lastIndexOf(FILE_SEPARATOR_STR)) {
			...
			if (firstFileSeparatorIndex > 0) {
				...
				return pathname.substring(0, 3) + ".." + FILE_SEPARATOR_STR + ".."
	...
	// --- Helper classes ------------------------------------------------------
	/***********************************************************
	 * Create a tooltip location directly over the menu item, * ie, left allign the
	 * tooltip text in "overlay" technique. *
	 ***********************************************************/
